...
// Auto-load the last calibrated photo from the IndexedDB cache, but only
// once the user is in Analyze - setCalibrating() hides the WebGL renderer,
// so restoring eagerly from Plan would hijack the Plan-mode view. Deferred
// past window 'load' so authoring/index.js's bootstrap has populated
// state.doc before checkRestoreAvailable() reads saved landmarks.
...
window.addEventListener('load', () => {
  ...
  if (railBtn && railBtn.classList.contains('active')) restoreCachedPhotoIfAny();
});

window.addEventListener('shell:mode', (e) => {
  if (e.detail?.mode !== 'analyze') return;
  if (!photoCanvas.hasPhoto()) { restoreCachedPhotoIfAny(); return; }
  photoCanvas.showIfLoaded();
  if (!isPhoto()) setCalibrating(true);
});
